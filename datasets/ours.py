...
class OurPseudoDataset(torch.utils.data.Dataset):
    ...
    def collate_fn(self, batch):
        ...
        return {
                ...
                'modal_mask': torch.stack([sample['modal_mask'] for sample in batch]),
    ...
    def __getitem__(self, idx):
        ...
        # if True:
            ...
            if h == self.image_size and w == self.image_size and np.random.rand() < 0.25:

                num_tries = 0
                bbox = mask_to_bbox(modal_mask[0].numpy())
                by0, bx0, w, h = bbox
                x_min, x_max = bx0, bx0 + h
                y_min, y_max = by0, by0 + w
                while True:
                    padding_type = np.random.randint(0, 8)
                    padding_mask = torch.zeros(self.image_size, self.image_size)

                    if padding_type == 0:
                        x = np.random.randint(0, x_max + 1)
                        padding_mask[:x, :] = 1
                    elif padding_type == 1:
                        x = np.random.randint(x_min, self.image_size)
                        padding_mask[x:, :] = 1
                    elif padding_type == 2:
                        y = np.random.randint(0, y_max + 1)
                        padding_mask[:, :y] = 1
                    elif padding_type == 3:
                        y = np.random.randint(y_min, self.image_size)
                        padding_mask[:, y:] = 1
                    elif padding_type == 4:
                        x = np.random.randint(0, x_max + 1)
                        y = np.random.randint(0, y_max + 1)
                        ...
                        x = np.random.randint(x_max, self.image_size)
                        y = np.random.randint(y_min, self.image_size)
                        padding_mask[x:, :] = 1
                        padding_mask[:, :y] = 1
                    elif padding_type == 6:
                    ...
                    _image = image * (1 - padding_mask[None]) + 1 * padding_mask[None]
                    _modal_mask = modal_mask * (1 - padding_mask)

                    if _modal_mask.sum() > 16 * 16:
                        break
            ...
            amodal_image[:, H:, :] = 1
            amodal_image[:, :, W:] = 1
            # print(f"Successfully got a valid sample at {idx} with type: {'mask' in annotation}")
            ...
            return {
                    ...
                    'caption': random.choice(annotation['captions']),
...
if __name__ == "__main__":
    ...
    for i in range(100):
        if True:
            data = dataset[i]
            image = data['image']
            modal_mask = data['modal_mask']
            amodal_image = data['amodal_image']
            amodal_mask = data['amodal_mask']
            save_image(image, f"data/ours_tmp/{i}_image.png")
            save_image(modal_mask, f"data/ours_tmp/{i}_modal_mask.png")
